...
demonstrated on reproducible simulations, including a measurement of the
deterministic layer's false-positive rate on a realistic ``messy-but-honest''
genuine population (5.5\% after a granularity-aware fix, with fraud recall held at
1.00); commercial operation, which triggers the heaviest regulatory duties, is
...
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt]
  ...
  catches internally inconsistent claims with an explainable, low-false-positive
  rule set---whose false-positive rate we measure on a realistic ``messy-but-honest''
  genuine population and reduce from 24.1\% to 5.5\% with a granularity-aware date
  rule (Section~\ref{sec:res:realistic})---together with an empirical demonstration
  that it---alone---cannot catch internally consistent AI fabrications, motivating
  the other layers.
...
incentive, and network properties on seeded simulations and a realistic
genuine-population false-positive measurement. We do \emph{not} claim a deployed
multi-organization study; that remains the central item of future work
(Section~\ref{sec:agenda}). We are also candid about a data-availability finding:
we attempted the false-positive measurement on two leading public corpora and
report why neither suffices---one lacks employment dates entirely, the other is
dominated by synthetic records with incoherent timelines
(Section~\ref{sec:res:realistic})---which is precisely why our headline
false-positive figure comes from a controlled, realistic generated population. We
are explicit throughout about which results are analytic, which are simulated, and
which depend on data we generate versus data we obtain.
...
explainable, parameter-light, and has a low false-positive rate---which we
quantify on a realistic genuine population in Section~\ref{sec:res:realistic}: it
fires only on self-contradiction, independent of lifestyle, geography, or digital
...
years) absorb benign transitions and rounding. A third tolerance is
granularity-aware: when a contested date boundary is given only at year
granularity (``2019'' with no month), sub-year ordering is unknowable, so the
overlap tolerance for that pair is widened by $\theta_g$ months (we use
$\theta_g=12$). As Section~\ref{sec:res:realistic} shows, this single rule is
responsible for most of the achievable false-positive reduction on real-world
r\'esum\'es, where year-only dates are common. Missing fields cause the
...
standard practice in fraud research without labels: draw a genuine population and
\emph{inject} controlled, labeled fraud. For the genuine population we use two
generated sources: a deterministically seeded, internally consistent population
(a correctness benchmark), and a \emph{realistic} ``messy-but-honest'' population
that injects the kinds of irregularity real r\'esum\'es exhibit---career gaps,
month- and year-granularity dates, short transition overlaps, claimed-year
rounding, and a documented minority with legitimately concurrent roles---which is
the instrument for our headline false-positive measurement. The AI-fabricated
positive class follows agent-based generators~\cite{agentgen2025}.

We also attempted the false-positive measurement on real public corpora and
report a data-availability finding (Table~\ref{tab:datasets}). The deterministic
checks reason over employment \emph{dates}, but
KARRIEREWEGE~\cite{karrierewege2025}, despite its scale (500k+ ESCO-linked
trajectories), stores only \emph{ordered occupation sequences} with no start/end
dates, age, or claimed-years, so none of the date-based checks can fire on it. A
dated technical-r\'esum\'e corpus that does carry employment dates yields an
implausible 65.9\% overlap false-positive rate, which diagnosis attributes to data
quality---byte-identical duplicate records, the same job title repeated with
independently-random overlapping date ranges, and an implausibly uniform
jobs-per-person distribution---rather than to engine behavior. Clean public
r\'esum\'e corpora with genuine, coherent employment dates are scarce (a privacy
reality); our released harness ingests one unchanged should it become available.

\begin{table}[t]\centering\small
\caption{Datasets used for evaluation.}\label{tab:datasets}
\begin{tabular}{lll}
\toprule
Dataset & Role & Source / outcome \\
\midrule
Seeded synthetic (this work) & correctness benchmark & seed tables + injection \\
Realistic population (this work) & \textbf{headline FP test} & messy-but-honest generator \\
Agent-fabricated (this work) & AI-fake positive class & \cite{agentgen2025}-style generator \\
KARRIEREWEGE~\cite{karrierewege2025} & attempted real FP & \emph{unusable: no dates} \\
Dated r\'esum\'e corpus & attempted real FP & \emph{unusable: synthetic/incoherent} \\
\bottomrule
\end{tabular}
\end{table}
...
difficulty; the decisive measurement is the false-positive rate on
\emph{atypical-but-genuine} r\'esum\'es, which we report in
Section~\ref{sec:res:realistic}.
...
\subsection{Realistic false positives}\label{sec:res:realistic}
The clean benchmark above understates difficulty by construction. We therefore
measure the deterministic layer on a \emph{realistic} genuine population
($n=3{,}000$ per seed, 20 seeds): non-fraudulent records that nonetheless carry
the irregularities of real r\'esum\'es---career gaps and sabbaticals, month- and
year-granularity dates, short legitimate transition overlaps, claimed-year
rounding, and a documented minority ($7\%$) with legitimately concurrent
full-time-coded roles (portfolio careers).

Under naive date parsing the false-positive rate is $24.1\%$ (95\% CI
$[23.7,24.4]$), almost entirely \code{EMPLOYMENT\_OVERLAP}. Diagnosis shows the
cause is \emph{phantom} overlaps: a job written as ``2019'' is read as January
2019, manufacturing an overlap with the prior job that did not exist. The
granularity-aware tolerance $\theta_g$ (Section~\ref{sec:det}) removes these,
cutting the false-positive rate to $5.5\%$ (95\% CI $[5.3,5.8]$) with
\emph{overlap-fraud recall held at $1.00$} under both settings
(Table~\ref{tab:realistic}, Figure~\ref{fig:realistic}). The residual $5.5\%$ is
attributable to genuinely concurrent employment: a sweep over its prevalence drives
the false-positive rate to $\approx 0$ when no concurrent roles are present and
upward monotonically as they become more common. This is the empirical basis for
treating the deterministic output as \emph{advisory and disputable}: its residual
false positives are exactly the cases (legitimate portfolio careers) that a
human---or a corroborating signal---should adjudicate, not the system alone.

\begin{table}[t]\centering\small
\caption{Deterministic false-positive rate on the realistic genuine population
(20 seeds, 95\% CI). The granularity-aware rule removes phantom overlaps from
year-only dates without any loss of fraud recall.}\label{tab:realistic}
\begin{tabular}{lcc}
\toprule
Setting & FP rate & Overlap-fraud recall \\
\midrule
Naive date parsing & 0.241 \,[0.237, 0.244] & 1.00 \\
Granularity-aware ($\theta_g{=}12$) & \textbf{0.055} \,[0.053, 0.058] & 1.00 \\
\bottomrule
\end{tabular}
\end{table}

\begin{figure}[t]\centering
\includegraphics[width=0.9\linewidth]{figures/realistic_fp.png}
\caption{Left: deterministic false-positive rate on realistic genuine r\'esum\'es,
naive vs.\ granularity-aware (fraud recall $=1.00$ in both). Right: the residual
false-positive rate is driven by the prevalence of legitimately concurrent
full-time roles, reaching $\approx 0$ when none are present.}\label{fig:realistic}
\end{figure}

We additionally ran the released harness on real public corpora and report a
data-availability finding (Section~\ref{sec:datasets}): KARRIEREWEGE has no
employment dates, so the date-based checks cannot fire; and a dated
technical-r\'esum\'e corpus is dominated by synthetic records whose incoherent
overlapping dates yield a $65.9\%$ rate that reflects data quality, not engine
behavior. A trustworthy real-data measurement awaits a public corpus with genuine,
coherent dates---or a consented deployment.
...
\paragraph{Construct validity.} Our genuine population is generated, not sampled
from deployment. The clean benchmark is deliberately consistent and therefore
\emph{understates} false positives; we address this with the realistic
``messy-but-honest'' population (Section~\ref{sec:res:realistic}), whose $5.5\%$
false-positive rate is our headline detection-cost figure. That figure is still a
generated estimate: lacking a public r\'esum\'e corpus with genuine, coherent
employment dates (Section~\ref{sec:datasets}), a deployment-grade measurement
across demographic and cultural cohorts remains future work, and our harness is
built to run it unchanged on any suitable corpus.
...
deterministic and inferential false-positive rates on a real r\'esum\'e corpus
with genuine, coherent employment dates and across demographic and cultural
cohorts (a fairness audit). As Section~\ref{sec:datasets} documents, such a corpus
is not currently available publicly---the leading candidates either lack dates or
are synthetically contaminated---so this measurement awaits either a suitable
public release or a consented deployment; our realistic generated population
(Section~\ref{sec:res:realistic}) is the present-day stand-in. On
